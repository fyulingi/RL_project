...
class MCTSAgent(Agent):
    ...
    def __init__(self, config, path, version, color, stochastic_steps, device):
...
class MentorAgent(Agent):
    ...
    def __init__(self, color):
        ...
        self.color = color
        self.ai = Mentorai(color)

    def play(self, board, last_move=None):

        return self.ai.action(board, self.color, 1)
    ...
    def self_play(self, degree=3):
        ...
        while game_result == "unfinished":
            last_move_record.append(last_move)
            action= self.ai.action(board, color, degree)
            board_record.append(board.copy())
            p = [0.0] * 225
            p[action] = 1.0
            p_record.append(p)
